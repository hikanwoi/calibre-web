    ...
    try:
        ...
        if response:
            ...
            if not response['deleted'] and response['file']['title'] == 'metadata (calibre-web).db' \
                ...
                copyfile(dbpath, os.path.join(tmp_dir, "metadata (calibre-web).db_" + str(current_milli_time())))
                log.info('Backing up existing and downloading updated metadata (calibre-web).db')
                gdriveutils.downloadFile(None, "metadata (calibre-web).db", os.path.join(tmp_dir, "tmp_metadata (calibre-web).db"))
                log.info('Setting up new DB')
                # prevent error on windows, as os.rename does on existing files, also allow cross hdd move
                move(os.path.join(tmp_dir, "tmp_metadata (calibre-web).db"), dbpath)
